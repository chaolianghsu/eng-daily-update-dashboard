...
function buildSystemPrompt(context) {
  ...
  return [
    ...
    layerGuidance,
    '- confidence 校準規則(從「中等信心 = 0.7」做加減,不要預設很低):',
    '    * 基準:REPO NOTES 有明確匹配 → 從 0.7 起跳。這是「還算有把握」的常態狀態。',
    '    * 加分到 0.8-0.9:多筆相似歷史 issue 指向同一 repo,且 issue 描述含具體技術線索(error message / API endpoint / 功能名)。',
    '    * 加分到 ≥ 0.9:內文明確指名 repo / endpoint / 模組,或錯誤訊息精確對應某個已知 repo。',
    '    * 減分到 0.5-0.6:issue 關鍵詞同時沾到多個 repo、描述偏短 / 偏模糊,但 REPO NOTES 仍可鎖定一個較可能的 repo。',
    '    * 減分到 < 0.5:真的沒頭緒(幾乎只有標題、關鍵字無法對應 REPO NOTES 任一條、多個 repo 同等可能)。這是少見狀況,不是預設。',
    '  (任何不確定性寫進 caveats,但不要用「我不太確定」當作壓低 confidence 的藉口 — 有 REPO NOTES 支撐就應該給 ≥ 0.7)。',
    '- reasoning 用 1-2 句繁體中文 (zh-TW) 說明判斷依據,必須明確引用 REPO NOTES 中對 top-1 repo 的職責描述。',
