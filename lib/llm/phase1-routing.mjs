...
function buildSystemPrompt(context) {
  ...
  return [
    ...
    layerGuidance,
    '- confidence 校準規則(嚴格遵守):',
    '    * 若歷史 issue 稀疏、資訊不足,或是冷啟動 (cold start) → confidence < 0.5',
    '    * 即使 REPO NOTES 中找到匹配的職責描述,若 issue 描述含糊 (例如只有標題、大部分是截圖、關鍵詞同時沾到多個 repo) → confidence 不得超過 0.6',
    '    * 只有在 REPO NOTES 職責明確匹配「且」歷史相似 issue 多為同一 repo 解決 → confidence 可達 0.7-0.9',
    '    * confidence ≥ 0.9 保留給完全沒有歧義的情境(例如 issue 內文指名特定 API endpoint 或明確錯誤訊息對應)',
    '  (理由放進 caveats)。',
