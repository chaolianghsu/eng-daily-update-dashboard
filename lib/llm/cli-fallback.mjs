...
const RETRYABLE_CODES = new Set(['cli_error', 'cli_timeout', 'cli_invalid_json']);
const DEFAULT_MAX_ATTEMPTS = 3;
const DEFAULT_BASE_DELAY_MS = 1000;
const DEFAULT_MAX_DELAY_MS = 10_000;

/**
 * Wraps `callClaudeCliWithTool` with exponential-backoff retry for transient
 * errors (rate-limit exit codes, timeouts, malformed JSON). Non-transient
 * errors (schema mismatch — the model answered but didn't match the contract)
 * are thrown immediately without retry.
 *
 * @param {object} params
 * @param {number} [params.maxAttempts=3]
 * @param {number} [params.baseDelayMs=1000]
 * @param {number} [params.maxDelayMs=10000]
 * @param {(ms:number) => Promise<void>} [params.sleep] - injectable for tests
 * @param {(info:{attempt:number, err:Error, delayMs:number}) => void} [params.onRetry] - telemetry
 * @param {...*} [params.rest] - forwarded to callClaudeCliWithTool
 */
export async function callClaudeCliWithToolRetrying({
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      return await callClaudeCliWithTool(cliParams);
    } catch (err) {
      lastErr = err;
      ...
      if (typeof onRetry === 'function') {
        try { onRetry({ attempt, err, delayMs }); } catch { /* swallow */ }
      }
      await sleep(delayMs);
    }
  }
