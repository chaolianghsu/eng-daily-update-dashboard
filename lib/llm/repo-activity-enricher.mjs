    ...
    while (queue.length > 0) {
      ...
      try {
        const [openMrs, recentCommits] = await Promise.all([
          fetchOpenMRsForRepo(apiCall, repo, mrLimit, warn),
          fetchRecentCommitsForRepo(apiCall, repo, sinceDate, commitLimit, warn),
        ]);
        byRepo[repo] = {
          open_mrs: openMrs,
...
async function fetchOpenMRsForRepo(apiCall, repo, limit, warn) {
  const encoded = encodeURIComponent(repo);
  const path = `/projects/${encoded}/merge_requests?state=opened&order_by=updated_at&per_page=${limit}`;
  let raw;
  try {
...
async function fetchRecentCommitsForRepo(apiCall, repo, sinceDate, limit, warn) {
  const encoded = encodeURIComponent(repo);
  const path = `/projects/${encoded}/repository/commits?since=${encodeURIComponent(sinceDate)}&per_page=${limit}`;
  let raw;
  try {
    raw = await apiCall(path);
  } catch (err) {
