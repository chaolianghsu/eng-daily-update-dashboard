...
// Tiered-ensemble ground truth extractor: MR cross-refs + closing-commenter
// commit heuristic (signal 2b) + LLM comment reader. Pure functions — callers
// wire IO.
//
...
//   extractAssigneeHeuristic({ issue, assigneeCommitsByRepo })   [DEPRECATED — signal 2a]
//   extractClosingCommenterHeuristic({ issue, comments, commenterCommitsByRepo })  [signal 2b]
//   identifyClosingCommenter(comments, { issueAuthor })
...
// ---- Signal 2a: Assignee heuristic (DEPRECATED) -----------------------------
//
// @deprecated — validation (2026-04-22, 135 K5 issues) showed 0% agreement with
// MR cross-refs. For this team, assignee = CSM / triage, not the fixer. Kept
// for backwards compat with existing call sites; always returns confidence none.
// New signal source: `extractClosingCommenterHeuristic` (signal 2b).
export function extractAssigneeHeuristic({ issue, assigneeCommitsByRepo } = {}) {
  ...
  return { repos, confidence: 'none', reason: 'deprecated: replaced by signal 2b (closing commenter)' };
}

// ---- Signal 2b: Closing-commenter commit heuristic --------------------------
//
// The LAST non-bot user comment before an issue closes is often written by the
// person who actually fixed it ("已更新至測試機", "已修復請測試", ...). We then
// check whether that commenter has commits in candidate repos in a window
// around `closed_at`. Wider window than signal 2a (±14d before, +1d after) to
// catch fixes that land well before the ticket formally closes.
...
function isBotUsername(username) {
  ...
  return BOT_USERNAME_PATTERNS.some((re) => re.test(username));
}

function commentAuthor(comment) {
  if (!comment || typeof comment !== 'object') return null;
  if (comment.author && typeof comment.author === 'object') {
    return comment.author.username ?? comment.author.name ?? null;
  }
  return comment.author_username ?? comment.author_name ?? null;
}

function commentTimestamp(comment) {
  const s = comment?.created_at ?? comment?.createdAt ?? null;
  if (!s) return null;
...
/**
 * Given a comments array (sorted ascending by created_at), identify the LAST
 * non-bot, non-system user comment. Returns { username, created_at } or null.
 * If `issueAuthor` is passed, comments by the issue reporter are still
 * considered (the reporter is often the CSM confirming the fix, but the
 * engineer may also post the closing "已更新").
 */
export function identifyClosingCommenter(comments, { issueAuthor } = {}) {
  if (!Array.isArray(comments) || comments.length === 0) return null;
  // Defensive copy + stable sort ascending by created_at.
  const sorted = [...comments].sort((a, b) => {
    const ta = commentTimestamp(a) ?? 0;
    const tb = commentTimestamp(b) ?? 0;
    return ta - tb;
  });
  for (let i = sorted.length - 1; i >= 0; i--) {
    const c = sorted[i];
    if (!c) continue;
    if (c.system === true) continue;
    const username = commentAuthor(c);
    if (!username) continue;
    if (isBotUsername(username)) continue;
    // Don't skip the issue author — they may be the one posting the fix
    // confirmation. issueAuthor is plumbed through for telemetry only.
    void issueAuthor;
    return { username, created_at: c.created_at ?? null };
  }
  return null;
}

const DAY_MS = 86400 * 1000;
const CLOSING_COMMENTER_WINDOW_BEFORE_MS = 14 * DAY_MS;
const CLOSING_COMMENTER_WINDOW_AFTER_MS = 1 * DAY_MS;

/**
 * Signal 2b — closing-commenter commit heuristic.
 *
 * @param {object} params
 * @param {object} params.issue - issue object with closed_at
 * @param {object[]} params.comments - notes array (system + user), sorted asc
 * @param {Object<string, Object<string, number[]>>} params.commenterCommitsByRepo
 *   Pre-fetched commit timestamps per repo per username:
 *     { "repoA/foo": { "joyce": [ts1, ts2], ... }, ... }
 *   where each `tsN` is a unix-millisecond timestamp.
 *
 * @returns {{
 *   repos: Set<string>,
 *   confidence: 'high'|'med'|'low'|'none',
 *   closing_commenter: string|null,
 *   reason: string
 * }}
 *
 * Confidence tiers:
 *   - `high`: closing commenter has ≥2 commits in a single repo within window
 *   - `med`:  exactly 1 commit in exactly 1 repo
 *   - `low`:  commits in ≥2 repos (ambiguous)
 *   - `none`: no closing commenter identified / no commit data / no commits in window
 */
export function extractClosingCommenterHeuristic({ issue, comments, commenterCommitsByRepo } = {}) {
  const repos = new Set();

  const closer = identifyClosingCommenter(comments, { issueAuthor: issue?.author?.username });
  if (!closer) {
    return { repos, confidence: 'none', closing_commenter: null, reason: 'no closing user comment' };
  }

  const username = closer.username;

  if (!commenterCommitsByRepo || typeof commenterCommitsByRepo !== 'object') {
    return {
      repos,
      confidence: 'none',
      closing_commenter: username,
      reason: 'no commit data',
    };
  }

  const closedAtRaw = issue?.closed_at ?? null;
  const closedAt = closedAtRaw ? new Date(closedAtRaw).getTime() : null;
  if (closedAt == null || Number.isNaN(closedAt)) {
    return {
      repos,
      confidence: 'none',
      closing_commenter: username,
  ...
  const windowStart = closedAt - CLOSING_COMMENTER_WINDOW_BEFORE_MS;
  const windowEnd = closedAt + CLOSING_COMMENTER_WINDOW_AFTER_MS;

  const lcUsername = username.toLowerCase();

  // Count commits per repo within the window, keyed by repo path.
  const perRepoCount = new Map();
  for (const [repoPath, authorMap] of Object.entries(commenterCommitsByRepo)) {
    if (!authorMap || typeof authorMap !== 'object') continue;
    // Allow case-insensitive author lookup.
    let timestamps = authorMap[username] ?? authorMap[lcUsername];
    if (!timestamps) {
      for (const k of Object.keys(authorMap)) {
        if (k.toLowerCase() === lcUsername) {
          timestamps = authorMap[k];
          break;
        }
      }
    }
    ...
    for (const ts of timestamps) {
      ...
      if (ts >= windowStart && ts <= windowEnd) count += 1;
    }
    if (count > 0) perRepoCount.set(repoPath, count);
  }

  if (perRepoCount.size === 0) {
    return {
      repos,
      confidence: 'none',
      closing_commenter: username,
      reason: `no commits in window for @${username}`,
    };
  }

  let confidence;
  if (perRepoCount.size >= 2) {
    confidence = 'low';
    for (const r of perRepoCount.keys()) repos.add(r);
  } else {
    // Exactly 1 repo
    const [[repo, count]] = [...perRepoCount.entries()];
    repos.add(repo);
    confidence = count >= 2 ? 'high' : 'med';
  }
  ...
  return {
    ...
    confidence,
    closing_commenter: username,
    reason: `@${username} has commits in ${perRepoCount.size} repo(s) within ±14d window`,
  };
}
  ...
  return {
    ...
    provenance: {
      ...
      assignee_heuristic: {
        ...
        repos: [...(signals?.assigneeHeuristic?.repos ?? [])],
        closing_commenter: signals?.assigneeHeuristic?.closing_commenter ?? null,
        reason:
